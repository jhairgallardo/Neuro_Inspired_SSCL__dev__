...
parser = argparse.ArgumentParser(description='Linear evaluation NO AUG')
# Dataset parameters
parser.add_argument('--data_path', type=str, default='/data/datasets/ImageNet-10')
parser.add_argument('--num_eval_classes', type=int, default=10)
...
def get_features(loader, encoder, device):
    encoder.eval()
    features = []
    targets = []
    with torch.no_grad():
...
def main():
    ...
    with open(os.path.join(args.save_dir, 'args.json'), 'w') as f: # save args
...
def main_worker(args, device):
    ...
    train_dataset_images = ClassIncremental(train_parent_dataset, increment=1, initial_increment=args.num_eval_classes, transformations=transform_noaug, class_order=data_class_order)[0]
    val_dataset_images = ClassIncremental(val_parent_dataset, increment=1, initial_increment=args.num_eval_classes, transformations=transform_noaug, class_order=data_class_order)[0]
    ...
    encoder = eval(args.enc_model_name)()
    if args.enc_pretrained_file_path is not None:
        state_dict = torch.load(args.enc_pretrained_file_path)
    ...
    ### Load top classifier
    classifier = nn.Linear(feat_dim, args.num_eval_classes).cuda()
    ...
    print('\n==> Extracting features')
    train_features, train_targets = get_features(train_loader_images, encoder, device)
    val_features, val_targets = get_features(val_loader_images, encoder, device)
