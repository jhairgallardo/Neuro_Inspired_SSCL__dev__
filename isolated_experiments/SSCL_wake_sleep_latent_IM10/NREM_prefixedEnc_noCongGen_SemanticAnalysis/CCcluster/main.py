...
parser = argparse.ArgumentParser(description='Semantic Memory training analysis - CCcluster')
# Dataset parameters
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_sem = ClusterLoss(class_num=args.num_pseudoclasses, temperature=args.tau, alpha=args.alpha, device=device).to(device)
        ### NREM Step ####
