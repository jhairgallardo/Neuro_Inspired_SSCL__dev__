...
parser.add_argument('--data_path', type=str, default="/data/datasets/CIFAR10")
...
parser.add_argument('--pca_epsilon', type=float, default=5e-4)
...
### Create save dir
save_dir = os.path.join(args.save_dir, f"{args.model_name}_cifar10")
if args.pca: save_dir += f"_pca_eps{args.pca_epsilon}"
...
transform_val = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(mean=mean, std=std)])
train_dataset = datasets.CIFAR10(args.data_path, train=True, transform=transform_train,  download=True)
val_dataset = datasets.CIFAR10(args.data_path, train=False, transform=transform_val, download=True)
...
### Load model
model = eval(args.model_name)(num_classes=args.num_classes, conv0_flag=args.pca, conv0_outchannels=54)

### Calculate pca filters and load it to the model
if args.pca:
    weight, bias = calculate_PCA_conv0_weights(model = model, dataset = train_dataset,
                                            save_dir = save_dir,nimg = 10000, 
                                            epsilon=args.pca_epsilon)
...
for epoch in range(args.epochs):
    ...
    if corrects_val/total_val > best_acc:
        ...
        torch.save(state_dict, os.path.join(save_dir, f"{args.model_name}_cifar10_best.pth"))
        text_print += " (Saved)"
    print(text_print), logging.info(text_print)
...
torch.save(state_dict, os.path.join(save_dir, f"{args.model_name}_cifar10_final.pth"))
