...
standard_expt = 'vgg11_bn_cifar10'
...
zca_expt = 'vgg11_bn_cifar10_zca_eps0.0005'
...
pca_expt = 'vgg11_bn_cifar10_pca_eps0.0005'
...
# Load saved stats zca normalized
zca_norm_expt = 'vgg11_bn_cifar10_zca_eps0.0005_normalize'
zca_norm_trainloss = np.load(f'{experiments_path}/{zca_norm_expt}/train_loss_all.npy')
zca_norm_trainacc = np.load(f'{experiments_path}/{zca_norm_expt}/train_accuracy_all.npy')
zca_norm_valloss = np.load(f'{experiments_path}/{zca_norm_expt}/val_loss_all.npy')
zca_norm_valacc = np.load(f'{experiments_path}/{zca_norm_expt}/val_accuracy_all.npy')
...
plt.plot(zca_norm_valloss, label='ZCA (10 filters) Normalized', color='purple', linewidth=2)
...
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/losses_all.png', bbox_inches='tight')
plt.close()
...
plt.plot(zca_norm_valacc, label='ZCA (10 filters) Normalized', color='purple', linewidth=2)
...
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/accuracies_all.png', bbox_inches='tight')
plt.close()
