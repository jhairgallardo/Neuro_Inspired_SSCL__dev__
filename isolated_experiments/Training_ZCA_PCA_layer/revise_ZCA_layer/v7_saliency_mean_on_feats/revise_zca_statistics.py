...
batch_abs_meanpool_saliency_maps/=batch_abs_meanpool_saliency_maps.sum(dim=(1, 2), keepdim=True)
batch_abs_l2pool_saliency_maps/=batch_abs_l2pool_saliency_maps.sum(dim=(1, 2), keepdim=True)

### Get crops for the batch
batch_abs_crops = smart_crop_batch(batch_abs_saliency_maps, num_crops=num_crops, scale=crop_scale)
batch_abs_meanpool_crops = smart_crop_batch(batch_abs_meanpool_saliency_maps, num_crops=num_crops, scale=crop_scale)
batch_abs_l2pool_crops = smart_crop_batch(batch_abs_l2pool_saliency_maps, num_crops=num_crops, scale=crop_scale)
...
# idx = 5 # 5 (bad) 8 (good) 23 (bad) 24 (nice)
    ...
    for n_crop in range(num_crops):
        plt.gca().add_patch(plt.Rectangle((batch_abs_crops[idx][n_crop][0], batch_abs_crops[idx][n_crop][1]), batch_abs_crops[idx][n_crop][2], batch_abs_crops[idx][n_crop][3], linewidth=2, edgecolor='r', facecolor='none'))
    ...
    plt.imshow(saliencymap_meanpool_image, cmap='jet', alpha=0.5)
    for n_crop in range(num_crops):
        plt.gca().add_patch(plt.Rectangle((batch_abs_meanpool_crops[idx][n_crop][0], batch_abs_meanpool_crops[idx][n_crop][1]), batch_abs_meanpool_crops[idx][n_crop][2], batch_abs_meanpool_crops[idx][n_crop][3], linewidth=2, edgecolor='r', facecolor='none'))
    ...
    plt.imshow(saliencymap_l2pool_image, cmap='jet', alpha=0.5)
    for n_crop in range(num_crops):
        plt.gca().add_patch(plt.Rectangle((batch_abs_l2pool_crops[idx][n_crop][0], batch_abs_l2pool_crops[idx][n_crop][1]), batch_abs_l2pool_crops[idx][n_crop][2], batch_abs_l2pool_crops[idx][n_crop][3], linewidth=2, edgecolor='r', facecolor='none'))
