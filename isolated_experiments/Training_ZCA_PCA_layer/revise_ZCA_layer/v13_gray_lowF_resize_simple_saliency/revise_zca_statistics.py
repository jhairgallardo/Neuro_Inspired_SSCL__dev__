...
def calculate_ZCA_conv0_weights(imgs, kernel_size=3, zca_epsilon=1e-6):
...
mean_imgs = zca_input_imgs.mean(dim=(0,2,3)).tolist()
with open(f'{save_dir}/mean_imgs_input_for_zca.json', 'w') as f:
    json.dump(mean_imgs, f)
weight = calculate_ZCA_conv0_weights(imgs=zca_input_imgs, kernel_size=conv0_kernel_size, zca_epsilon=zca_epsilon)
...
### Calculate high frequency gray filters
weight_gray = weight.mean(dim=0, keepdim=True)
# plot_filters_and_hist(weight_gray, name='zca_filters_gray', save_dir=save_dir)

### Calculate high frequency gray filters
weight_gray_lf = weight_lf.mean(dim=0, keepdim=True)
# plot_filters_and_hist(weight_gray_lf, name='zca_filters_gray_lf', save_dir=save_dir)
...
if zca_scale_filter:
    ...
    # scale down high frequency filters for grayscale
    aux_conv0 = torch.nn.Conv2d(3, weight_gray.shape[0], kernel_size=conv0_kernel_size, stride=1, padding='same', padding_mode='replicate', bias=False)
    aux_conv0.weight = torch.nn.Parameter(weight_gray)
    aux_conv0.weight.requires_grad = False
    weight_gray = scaled_filters(aux_conv0, imgs=zca_input_imgs)
    del aux_conv0

    # scale down low frequency filters for grayscale
    aux_conv0 = torch.nn.Conv2d(3, weight_gray_lf.shape[0], kernel_size=conv0_kernel_size, stride=1, padding='same', padding_mode='replicate', bias=False)
...
# idx = 5 # 5 (bad) 8 (good) 23 (bad) 24 (nice)
    ...
    # Plot all feature maps of idx after activation
    feats_act = batch_zca_act_out[idx]
    num_feats = feats_act.shape[0]
    figsize=(6*(num_feats/2), 10)
    plt.figure(figsize=figsize)
    for j in range(feats_act.shape[0]):
        plt.subplot(1,num_feats,j+1)
        plt.imshow(feats_act[j].cpu().detach().numpy())
        plt.axis('off')
    plt.savefig(os.path.join(save_dir_saliency,f"image_{idx}_zcafeats_act.png"), bbox_inches='tight', dpi=300)
    plt.close()
