...
def scaled_filters(zca_layer, imgs, save_dir=None, prob_threshold=0.99999, name='maxscaled'):
    ...
    plot_filters_and_hist(zca_layer.weight, name=f'zca_filters_{name}', save_dir=save_dir)
    plot_channel_cov(zca_layer, imgs, name=f'zcainput_channelcov_zca_{name}_out', save_dir=save_dir)
    plot_zca_layer_output_hist(zca_layer, imgs, name=f'zcainput_zca_{name}_out_hist', save_dir=save_dir)
    ...
    plot_cdf_with_threshold(cdf, bins_count, threshold, prob_threshold, multiplier, save_dir, name=f'zcainput_zca_{name}_out_cdf')
...
def compute_saliency_map_ggd_batch(features, ggd_params, weighted=False):
    ...
    for i in range(num_filters):
        ...
        if weighted:
            ...
            gamma_func = torch.exp(torch.special.gammaln(theta_inv)) # same as scipy.special.gamma(theta_inv)
...
plot_filters_and_hist(conv0.weight, name='zca_filters_raw_hf', save_dir=save_dir)
plot_channel_cov(conv0, zca_input_imgs, name='zcainput_channelcov_zca_raw_hf_out', save_dir=save_dir)
plot_zca_layer_output_hist(conv0, zca_input_imgs, name='zcainput_zca_raw_hf_out_hist', save_dir=save_dir)

### Downscale high frequency filters
if zca_scale_filter:
    weight = scaled_filters(conv0, imgs=zca_input_imgs, save_dir=save_dir, name='maxscaled_hf')
    conv0.weight = torch.nn.Parameter(weight)
    conv0.weight.requires_grad = False
### Plot stats for scaled main 3 filters
plot_filters_and_hist(conv0.weight, name='zca_filters_multiplier_hf', save_dir=save_dir)
plot_channel_cov(conv0, zca_input_imgs, name='zcainput_channelcov_zca_multiplier_hf_out', save_dir=save_dir)
plot_zca_layer_output_hist(conv0, zca_input_imgs, name='zcainput_zca_multiplier_hf_out_hist', save_dir=save_dir)

### Create another 3 filters that are low frequency
weight_lf = F.interpolate(weight, size=conv0_kernel_size+14, mode='bilinear', align_corners=True) # kernel5 (+4), kernel7 (+14)
weight_lf = weight_lf[:,:,7:-7,7:-7] # take the center of the low frequency filters to match the high frequency filters size (conv0_kernel_size)
conv0 = nn.Conv2d(3, weight_lf.shape[0], kernel_size=weight_lf.shape[2], stride=1, padding='same', padding_mode='replicate', bias=False)
conv0.weight = torch.nn.Parameter(weight_lf)
conv0.weight.requires_grad = False
plot_filters_and_hist(conv0.weight, name='zca_filters_raw_lf', save_dir=save_dir)
plot_channel_cov(conv0, zca_input_imgs, name='zcainput_channelcov_zca_raw_lf_out', save_dir=save_dir)
plot_zca_layer_output_hist(conv0, zca_input_imgs, name='zcainput_zca_raw_lf_out_hist', save_dir=save_dir)

### Downscale low frequency filters
if zca_scale_filter:
    weight_lf = scaled_filters(conv0, imgs=zca_input_imgs, save_dir=save_dir, name='maxscaled_lf')
    conv0.weight = torch.nn.Parameter(weight_lf)
    conv0.weight.requires_grad = False
### Plot stats for scaled low frequency filters
plot_filters_and_hist(conv0.weight, name='zca_filters_multiplier_lf', save_dir=save_dir)
plot_channel_cov(conv0, zca_input_imgs, name='zcainput_channelcov_zca_multiplier_lf_out', save_dir=save_dir)
plot_zca_layer_output_hist(conv0, zca_input_imgs, name='zcainput_zca_multiplier_lf_out_hist', save_dir=save_dir)
