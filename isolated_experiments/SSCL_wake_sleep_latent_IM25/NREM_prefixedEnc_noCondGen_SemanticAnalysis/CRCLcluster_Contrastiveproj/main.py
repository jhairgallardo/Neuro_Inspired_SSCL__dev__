...
from models import *
from loss_functions import CRCL_PCLoss, CRCL_FCLoss
...
parser = argparse.ArgumentParser(description='Semantic Memory training analysis - CRCLcluster + Contrastiveproj')
# Dataset parameters
...
parser.add_argument('--tau_inst', type=float, default=0.1)
# Training parameters
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_instanceloss = CRCL_FCLoss(batch_size=args.episode_batch_size, temperature=args.tau_inst, device=device).to(device)
        ### NREM Step ####
        print(f"### NREM step -- task {task_id+1} ##")
        WS_trainer.NREM_sleep(optimizers = [optimizer_sem], 
                            schedulers = [scheduler_sem],
                            criterions = [criterion_sem, criterion_instanceloss],
                            task_id = task_id,
                            scaler=scaler,
                            writer = writer)
