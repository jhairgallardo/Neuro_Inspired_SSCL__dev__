...
class Semantic_Memory_Model(torch.nn.Module):
    def __init__(self, features_dim, num_pseudoclasses, proj_dim=2048, out_dim=1024):
        ...
        self.instance_projector = nn.Sequential(
            conv1x1(self.features_dim, self.features_dim, bias=True),
    ...
    def forward(self, x, return_inst=False):
        ...
        x = self.cluster_projector(x_inst) # x_inst.detach()?
        x = torch.flatten(x, 1)
        x = F.normalize(x, dim=1)
        x_cluster = self.linear_head(x)
        if return_inst:
            return normalize(torch.flatten(x_inst, 1), dim=1), x_cluster
        else:
            return x_cluster
