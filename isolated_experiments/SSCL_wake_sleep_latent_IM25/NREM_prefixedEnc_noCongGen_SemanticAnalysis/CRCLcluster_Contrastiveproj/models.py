...
class Semantic_Memory_Model(torch.nn.Module):
    def __init__(self, features_dim, num_pseudoclasses):
        super().__init__()
        ...
        #### Instance Projector (Q)
        self.instance_projector = nn.Sequential(
            nn.Linear(self.features_dim, self.features_dim),
            torch.nn.GroupNorm(32, self.features_dim),
            torch.nn.Mish(),
            nn.Linear(self.features_dim, self.features_dim),
        )
        ...
        self.cluster_projector = torch.nn.Sequential(
            ...
            nn.Mish(),
            nn.Linear(self.features_dim, self.num_pseudoclasses),
            nn.Softmax(dim=1)
        )

        for m in self.instance_projector.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='leaky_relu')
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
                nn.init.constant_(m.weight, 1)
        ...
        for m in self.cluster_projector.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='leaky_relu')
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
                nn.init.constant_(m.weight, 1)
    ...
    def forward(self, x, return_inst=False):
        x = self.pool(x)
        x = torch.flatten(x, 1)
        x_inst = self.instance_projector(x)
        x_cluster = self.cluster_projector(x_inst)
        if return_inst:
            return normalize(x_inst, dim=1), x_cluster
        else:
            return x_cluster
