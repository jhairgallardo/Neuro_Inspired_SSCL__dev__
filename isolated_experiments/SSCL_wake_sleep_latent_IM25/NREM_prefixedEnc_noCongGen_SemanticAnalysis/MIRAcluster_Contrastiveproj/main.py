...
from models import *
from loss_functions import SwapLossViewExpanded, InstanceLoss
...
parser = argparse.ArgumentParser(description='Semantic Memory training analysis - MIRAcluster + Contrastiveproj')
# Dataset parameters
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_instloss = InstanceLoss(batch_size = args.episode_batch_size, temperature = args.tau_inst, device = device)
        ### NREM Step ####
        print(f"### NREM step -- task {task_id+1} ##")
        WS_trainer.NREM_sleep(optimizers = [optimizer_sem], 
                            schedulers = [scheduler_sem],
                            criterions = [criterion_crossentropyswap, criterion_instloss],
                            task_id = task_id,
                            scaler=scaler,
                            writer = writer)
        ...
        for task_id_aux in range(task_id+1):
            ...
            task_id_val_stats = evaluate_semantic_memory(task_id_val, view_encoder, semantic_memory, args.tau_s, args.num_pseudoclasses, None, device, False, args.num_classes, None)
