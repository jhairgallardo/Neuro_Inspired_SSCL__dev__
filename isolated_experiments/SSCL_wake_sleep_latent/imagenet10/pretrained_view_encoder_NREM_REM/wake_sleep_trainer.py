...
class Wake_Sleep_trainer:
    ...
    def sleep_phase_NREM(self, 
        ...
        '''
        Non-REM: Train semantic memory and generator using episodic memory
        '''
        ...
        for i in range(0, num_episodes_per_sleep, self.episode_batch_size):
            ...
            if i==0 or (i//self.episode_batch_size) % 10 == 0:
                ...
                print(f'NREM: Episode [{current_episode_idx}/{num_episodes_per_sleep}] ' +
    ...
    def sleep_phase_REM(self, 
                        num_episodes_per_sleep, 
                        optimizer_semantic_memory, criterion_semantic_memory, scheduler_semantic_memory, 
                        device, writer=None, task_id=None):
        '''
        REM: Train semantic memory using episodic memory (View encoder is pretrained)
        '''
        ...
        for i in range(0, num_episodes_per_sleep, self.episode_batch_size):
            ...
            # Forward pass on generator, encoder and semantic memory
            batch_logits = self.semantic_memory(self.view_encoder(self.view_generator(batch_embeddings)))
            ...
            if i==0 or (i//self.episode_batch_size) % 10 == 0:
                current_episode_idx = min(i+self.episode_batch_size,num_episodes_per_sleep)
                print(f'REM: Episode [{current_episode_idx}/{num_episodes_per_sleep}] ' +
                      f'-- SM lr: {scheduler_semantic_memory.get_last_lr()[0]:.6f} -- Consis: {consis_loss.item():.6f} -- Sharp: {sharp_loss.item():.6f} -- Div: {div_loss.item():.6f} -- SM Total loss: {semantic_loss.item():.6f}')
                
                if writer is not None and task_id is not None:
                    writer.add_scalar('REM Consistency Loss', consis_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
                    writer.add_scalar('REM Sharpness Loss', sharp_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
                    writer.add_scalar('REM Diversity Loss', div_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
                    writer.add_scalar('REM SM Total Loss', semantic_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
