...
class ResNet(nn.Module):
    def __init__(
        ...
        conv0_kernel_size=5,
        get_tensor_before_avgpool=False
    ) -> None:
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.GroupNorm
        ...
        self.get_tensor_before_avgpool = get_tensor_before_avgpool
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.get_tensor_before_avgpool:
            return x
...
class ResNetDec(nn.Module):
    ...
    ## Ref: https://github.com/karpathy/minGPT/blob/3ed14b2cec0dfdad3f4b2831f2b4a86d11aef150/mingpt/model.py#L136
    def get_parameters_controlled_wd(self, wd, lr):
        """
        ...
        blacklist_weight_modules = (nn.GroupNorm, )
        ...
        optim_groups = [
            {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": wd, 'lr':lr},
            {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0, 'lr':lr},
        ]
...
def resnet18dec(num_Blocks=[1,1,1,1], nc=3) -> ResNetDec:
    return ResNetDec(num_Blocks=num_Blocks, nc=nc)