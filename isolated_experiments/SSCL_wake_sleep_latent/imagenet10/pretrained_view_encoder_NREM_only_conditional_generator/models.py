...
class ResNetDec(nn.Module):
    def __init__(self, num_Blocks=[1,1,1,1], nc=3):
        super().__init__()
        self.in_planes = 513 #512 + 1 for Conditioning
        ...
        ## MLP for bbox conditioning
        self.mlp_bbox = nn.Linear(4, 49) # 4 > 7x7=49
    ...
    def forward(self, x, bbox):
        # Create bbox conditioned channel
        bbox_cond = self.mlp_bbox(bbox) # Nx49
        bbox_cond = torch.unsqueeze(bbox_cond, 1) # Nx49 >> Nx1x49
        bbox_cond = torch.reshape(bbox_cond, (bbox_cond.shape[0], 1, 7, 7)) # Nx1x49 >> Nx1x7x7

        # Concatenate conditions
        x = torch.cat((x, bbox_cond), 1) # Nx513x7x7
