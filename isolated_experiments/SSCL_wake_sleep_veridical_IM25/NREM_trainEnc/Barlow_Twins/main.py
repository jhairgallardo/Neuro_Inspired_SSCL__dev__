...
parser = argparse.ArgumentParser(description='NREM train encoder wake_sleep - Barlow Twins')
# Dataset parameters
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_barlow = BarlowLossViewExpanded(num_views = args.num_views).to(device)
        ...
        WS_trainer.sleep(view_encoder,
                        ...
                        criterions = [criterion_barlow, criterion_koleo],
                        task_id = task_id,
                        scaler=scaler,
                        writer = writer)
