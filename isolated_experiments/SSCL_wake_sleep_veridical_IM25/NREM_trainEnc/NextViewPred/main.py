...
parser = argparse.ArgumentParser(description='NREM train encoder wake_sleep - NextViewPred')
# Dataset parameters
...
def main():
    ...
    ### Save predictor at random init
    predictor_state_dict = predictor_net.module.state_dict()
    torch.save(predictor_state_dict, os.path.join(args.save_dir, f'predictor_taskid_randinit.pth'))
    ...
    for task_id in range(len(train_tasks)):
        ...
        ######-- SLEEP PHASE --######
        print("\n######-- Sleep Phase --######")
        param_groups_rep_learning = [{'params': view_encoder.parameters(), 'lr': args.lr, 'weight_decay': args.wd},
                                    {'params': predictor_net.parameters(), 'lr': args.lr, 'weight_decay': args.wd}]
        
        optimizer = torch.optim.AdamW(param_groups_rep_learning, lr = 0, weight_decay = 0)
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr = [args.lr, args.lr], steps_per_epoch = args.num_batch_episodes_per_sleep, epochs=1)
        ...
        ### Save predictor
        predictor_state_dict = predictor_net.module.state_dict()
        torch.save(predictor_state_dict, os.path.join(args.save_dir, f'predictor_taskid_{task_id}.pth'))
