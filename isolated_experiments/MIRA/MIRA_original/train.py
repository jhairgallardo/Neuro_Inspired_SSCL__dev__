...
class Trainer(object):
    ...
    def get_args_parser():
        ...
        # Benchmark mode
        parser.add_argument('--benchmark', action='store_true', help='set cudnn.deterministic=True', default=True) ############### No default

        # Machine settings
        parser.add_argument('--seed', default=5678, type=int, help='seed') ################### 5678
        parser.add_argument('--data-dir', default='/data/datasets/ImageNet-10', help='path to dataset')
        parser.add_argument('--workers', default=8, type=int, help='total number of data loading workers') ############### 64
        ...
        parser.add_argument('--distributed', default=False, type=lambda s: s.lower() in ['true', 't', 'yes', '1']) ############### True

        # Wandb logging
        parser.add_argument("--name", default='mira', type=str)
        parser.add_argument("--entity", default='jhairgallardo', type=str)
        parser.add_argument("--project", default='MIRA_original', type=str)

        # Training
        parser.add_argument('--epochs', default=400, type=int, help='number of epochs')
        parser.add_argument('--batch-size', default=128, type=int, help='batch-size of all GPUs on the current node.') ############### 4096
        parser.add_argument('--freeze-head-epochs', default=1, type=int)

        # Multi-crop augmentation
        parser.add_argument('--num-local-crops', default=0, type=int) ############### 6
        ...
        # Hyper-parameters
        parser.add_argument('--num-prototypes', default=30, type=int) ############### 3000
        ...
        parser.add_argument('--beta-initial', default=0.75, type=float) ####################### 0.75
        parser.add_argument('--beta-final', default=0.75, type=lambda s: convert_to_float(s)) ####################### 2./3.
    ...
    def train_loop(self, epoch):
        ...
        for i, batch in train_loader:
            # training step
            outs = self._train_step(batch, epoch, i)
            self._step += 1

            # # logger update
            self.logger.log(outs)
    ...
    def _train_step(self, batch, epoch, i):
        x, targets = batch
        ...
        # mixed precision
        with autocast():
            outs = self.model(x, momentum=False, m=self.ema_momentum) ############# True
            outs['targets'] = [mira(k=_k,
                                    tau=self.args.tau_t,
                                    beta=self.args.beta,
                                    iters=self.args.iters) for _k in outs['q']] ########### 'k'  (using 1 is not doing ema)
            # loss
        ...
        # plot outs and targets
        if i%50 == 0:
            image_probs = F.softmax(outs['q'][0][0], dim=0).detach().cpu().numpy()
            image_probs_sharp = F.softmax(outs['q'][0][0]/self.args.tau_s, dim=0).detach().cpu().numpy()
            image_pseudolabel = outs['targets'][0][0].detach().cpu().numpy()

            os.makedirs('images_probs_pseudolabels', exist_ok=True)
            ...
            plt.savefig(os.path.join('images_probs_pseudolabels', f'probabilities_pseudolabel_epoch{epoch}.png'), bbox_inches='tight')
            plt.close()
