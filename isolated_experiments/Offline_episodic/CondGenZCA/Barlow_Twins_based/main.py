...
from models_encBNMish_CondGen_zca import *
...
# ZCA layer parameters (pretrained)
parser.add_argument('--zca_pretrained_file_path', type=str)
...
def main():
    ...
    print('\n==> Load ZCA layer with pre-trained weights')
    assert args.zca_pretrained_file_path is not None
    zca_state_dict = torch.load(args.zca_pretrained_file_path)
    ...
    zca_layer = zca_layer.to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (batch_episode, _) in enumerate(train_loader):
            ...
            for v in range(args.num_views):
                ...
                with autocast():
                    ...
                    batch_gen_zcaimages, batch_gen_FTtensors = cond_generator(batch_first_view_tensors, batch_actions)
                    batch_gen_DecEnctensors = view_encoder(batch_gen_zcaimages)

                    # Direct forward pass (skip FTN to boost training of generator)
                    batch_gen_zcaimages_direct = cond_generator(batch_tensors, None, skip_FTN=True)
                    batch_gen_DecEnctensors_direct = view_encoder(batch_gen_zcaimages_direct)
        ...
        if (epoch+1) % 5 == 0 or epoch==0:
            ...
            episodes_plot_zcaimgs = torch.empty(0)
            episodes_plot_gen_zcaimgs = torch.empty(0)
            with torch.no_grad():
                first_view_tensors = view_encoder(zca_layer(episodes_plot_imgs[:,0]))
                for v in range(args.num_views):
                    imgs = episodes_plot_imgs[:,v]
                    actions = episodes_plot_actions[:,v]
                    zcaimages = zca_layer(imgs)
                    gen_zcaimages, _ = cond_generator(first_view_tensors, actions)
                    episodes_plot_zcaimgs = torch.cat([episodes_plot_zcaimgs, zcaimages.unsqueeze(1).detach().cpu()], dim=1)
                    episodes_plot_gen_zcaimgs = torch.cat([episodes_plot_gen_zcaimgs, gen_zcaimages.unsqueeze(1).detach().cpu()], dim=1)
            episodes_plot_imgs = episodes_plot_imgs.detach().cpu()
            # plot each episode (grid with 3 rows: original images, zca images, generated zca images)
            for i in range(n):
                ...
                episode_i_zcaimgs = episodes_plot_zcaimgs[i]
                # zca images should normalze to be from 0 to 1 (not using mean and std, just do -min and divided by max. Do this to each image)
                for j in range(episode_i_zcaimgs.shape[0]):
                    img = episode_i_zcaimgs[j]
                    img = (img - img.min()) / (img.max() - img.min())
                    episode_i_zcaimgs[j] = img

                episode_i_gen_zcaimgs = episodes_plot_gen_zcaimgs[i]
                # gen zca images should normalze to be from 0 to 1 (not using mean and std, just do -min and divided by max. Do this to each image)
                for j in range(episode_i_gen_zcaimgs.shape[0]):
                    img = episode_i_gen_zcaimgs[j]
                    img = (img - img.min()) / (img.max() - img.min())
                    episode_i_gen_zcaimgs[j] = img

                grid = torchvision.utils.make_grid(torch.cat([episode_i_imgs, episode_i_zcaimgs, episode_i_gen_zcaimgs], dim=0), nrow=args.num_views)
