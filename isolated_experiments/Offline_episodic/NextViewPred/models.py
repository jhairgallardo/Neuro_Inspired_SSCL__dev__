...
class ConditionedPredictor(nn.Module):
    def __init__(self, 
                 ...
                 num_layers=6, 
                 nhead=8, 
                 dim_feedforward=2048, # it should always be hidden_dim * 4
                 dropout=0.1,
                 sequence_length=50 # 1 action token + 49 feature tokens (7x7)
                 ):
        ...
        self.action_mlp = nn.Sequential(
            nn.Linear(self.action_code_dim, self.hidden_dim),
            nn.ReLU(),
        ...
        self.feature_mlp = nn.Linear(self.input_dim, self.hidden_dim)

        # Positional Encoding for the sequence
        self.positional_encoding = PositionalEncoding(d_model=self.hidden_dim, max_len=self.sequence_length)

        # Transformer Encoder
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.hidden_dim, nhead=nhead,
                                                   dim_feedforward=dim_feedforward, dropout=dropout,
                                                   activation='gelu')
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        
        # Output MLP if the output dimension is different from the hidden dimension
    ...
    def forward(self, feature_map, action_code):
        ...
        action_token = self.action_mlp(action_code)  # shape: (batch_size, hidden_dim)
        action_token = action_token.unsqueeze(1)  # shape: (batch_size, 1, hidden_dim)
        ...
        feature_tokens = self.feature_mlp(feature_tokens) # shape (batch_size, 49, hidden_dim)

        # Step 3: Concatenate the action token with feature tokens (action token first)
        tokens = torch.cat((action_token, feature_tokens), dim=1)  # shape: (batch_size, 50, hidden_dim)

        # Step 4: Add positional encoding
        tokens = self.positional_encoding(tokens)

        # Step 5: Prepare tokens for Transformer (sequence length first)
        tokens = tokens.permute(1, 0, 2)  # shape: (50, batch_size, hidden_dim)

        # Step 6: Pass through the Transformer Encoder
        transformed_tokens = self.transformer_encoder(tokens)  # shape: (50, batch_size, hidden_dim)

        # Step 7: drop first token and prepare for output
        transformed_tokens = transformed_tokens[1:, :, :].permute(1, 0, 2)  # shape: (batch_size, 49, hidden_dim)
        transformed_feature_map = self.output_mlp(transformed_tokens) # shape: (batch_size, 49, output_dim)
        
        # Step 8: Reshape and apply activation function
        transformed_feature_tokens = transformed_tokens.permute(0, 2, 1) # shape: (batch_size, output_dim, 49)
        transformed_feature_map = transformed_feature_tokens.view(batch_size, self.output_dim, 7, 7)  # shape: (batch_size, output_dim, 7, 7)
