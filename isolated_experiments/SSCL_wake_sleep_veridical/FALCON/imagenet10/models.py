...
class Semantic_Memory_Model(torch.nn.Module):
    def __init__(self, encoder, num_pseudoclasses):
        ...
        #### Projector (R) FALCON BN
        self.projector = torch.nn.Sequential(
            nn.Linear(self.features_dim, self.features_dim, bias=True),
            nn.BatchNorm1d(self.features_dim),
        )
        #### Projector (R) FALCON GN+WS
        # self.projector = torch.nn.Sequential(
        #     conv1x1_WS(self.features_dim, self.features_dim), # bias is false. WS
        #     nn.GroupNorm(32, self.features_dim),
        # )

        #### Linear head (F) FALCON
        self.linear_head = Falcon_linear_head(self.features_dim, self.num_pseudoclasses)

    def forward(self, x, proj_out=False):

        # forward when using BN in projector
        x_enc = self.encoder(x)
        x_proj = self.projector(x_enc)
        x_proj = F.normalize(x_proj, dim=1)
        x_proj = x_proj * np.sqrt(x_proj.shape[1] / x_proj.shape[0]) ## sqrt(f / n)
        x_out = self.linear_head(x_proj)
...
class Falcon_linear_head(nn.Module):
    def __init__(self, num_features, num_classes, *args, **kwargs):
        super().__init__()
        self.num_classes = num_classes

        base = torch.zeros(num_classes, num_features) # c x f
        for i in range(num_classes):
            v = 2 * bernoulli.rvs(0.5, size=num_features) - 1 ## Rademacher distribution
            base[i] = torch.tensor(v, dtype=torch.float32)
