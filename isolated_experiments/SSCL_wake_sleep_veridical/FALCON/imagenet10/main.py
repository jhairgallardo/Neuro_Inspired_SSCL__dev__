...
from loss_functions import CrossEntropyLossViewExpanded, KLLossViewExpanded
...
parser = argparse.ArgumentParser(description='SSCL FALCON Wake-Sleep veridical')
# Dataset parameters
...
parser.add_argument('--num_pseudoclasses', type=int, default=4096)

parser.add_argument('--lr', type=float, default=1e-3) #  LARS: 0.15, 0.3 # AdamW 0.001
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_crossentropy = CrossEntropyLossViewExpanded(num_views = args.num_views).to(device)
        criterion_KL = KLLossViewExpanded(num_views = args.num_views).to(device)
        ...
        WS_trainer.sleep_phase(num_episodes_per_sleep = args.num_episodes_per_sleep,
                               optimizer = optimizer, 
                               criterions = [criterion_crossentropy, 
                                             criterion_KL], 
                               scheduler = scheduler,
                               device = device,
