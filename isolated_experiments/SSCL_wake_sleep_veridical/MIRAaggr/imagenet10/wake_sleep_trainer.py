...
class Wake_Sleep_trainer:
    def __init__(self, model, episode_batch_size, args=None):
        ...
        self.args = args
        self.tau_t = 0.225 # 0.8 0.225
        self.tau_s = 0.1 # 1.0 0.1
        self.beta = 0.75 # 2./3.
    ...
    def sleep_phase(self, 
        ...
        criterion_consistencyskl = criterions[4]
        ...
        for i in range(0, num_episodes_per_sleep, self.episode_batch_size):
            ...
            #### Aggregate output logits ####
            batch_logits_aggregated = batch_logits.mean(dim=1) # mean
            # batch_logits_aggregated = batch_logits.max(dim=1)[0] # max
            # weighted_exp = torch.exp(8 * batch_logits) # boltzman operator (smoothmax)
            # batch_logits_aggregated = torch.sum(batch_logits * weighted_exp, dim=1) / torch.sum(weighted_exp, dim=1)  # Sum over N*H*W, result shape [C]

            #### Aggregate projector logits ####
            # batch_proj_logits_aggregated = batch_proj_logits.mean(dim=1) # mean
            # batch_proj_logits_aggregated = batch_proj_logits.max(dim=1)[0] # max
            weighted_exp = torch.exp(4 * batch_proj_logits) # boltzman operator (smoothmax)
            batch_proj_logits_aggregated = torch.sum(batch_proj_logits * weighted_exp, dim=1) / torch.sum(weighted_exp, dim=1)  # Sum over N*H*W, result shape [C]
            ...
            # batch_labels = batch_labels[:,0] # 1 label per episode (I will agregate the episode logits)
            # batch_labels = batch_labels.to(device) # all episodes labels and all views in one batch of (b v)

            # MIRA pseudolabels (Use only the first view -> unchanged view) # 1 label per episode (I will agregate the episode logits)
            batch_labels = mira_pseudolabeling(batch_logits = batch_logits,
                                               num_views=num_views,
                                               tau=self.tau_t, 
                                               beta=self.beta, 
                                               iters=30)
            batch_labels = batch_labels[:,0] # 1 label per episode. (Take the first view label)

            optimizer.zero_grad()

            #### Losses ####
            crossentropy_loss = criterion_crossentropy(batch_logits_aggregated/self.tau_s, batch_labels)
            consistency_carl_loss = criterion_consistencycarl(batch_logits/self.tau_s)
            # consistency_mse_loss = criterion_consistencymse(batch_logits/self.tau_s)
            # consistency_skl_loss = criterion_consistencyskl(batch_logits/self.tau_s)
            koleo_loss = criterion_koleo(batch_proj_logits_aggregated)
            
            #### Total Loss ####
            # loss = crossentropy_loss
            # loss = crossentropy_loss + consistency_carl_loss
            loss = crossentropy_loss + consistency_carl_loss + koleo_loss
            ...
            if i==0 or (i//self.episode_batch_size) % 5 == 0 or i==num_episodes_per_sleep-self.episode_batch_size:
                ...
                print(f'Episode [{current_episode_idx}/{num_episodes_per_sleep}] -- lr: {scheduler.get_last_lr()[0]:.6f}' +
                      f' -- mi_ps: {mi_ps.item():.6f} -- mi_pt: {mi_pt.item():.4f}' +
                      f' -- CrossEntropy: {crossentropy_loss.item():.4f}' +
                      f' -- ConsistencyCarl: {consistency_carl_loss.item():.4f}' +
                    #   f' -- ConsistencyMSE: {consistency_mse_loss.item():.4f}' +
                    #   f' -- ConsistencySKL: {consistency_skl_loss.item():.4f}' +
                ...
                if writer is not None and task_id is not None:
                    ...
                    writer.add_scalar('ConsistencyCarl Loss', consistency_carl_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
                    # writer.add_scalar('ConsistencyMSE Loss', consistency_mse_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
                    # writer.add_scalar('ConsistencySKL Loss', consistency_skl_loss.item(), task_id*num_episodes_per_sleep + current_episode_idx)
            ...
            if self.args is not None and (i==0 or (i//self.episode_batch_size) % 100 == 0 or i==num_episodes_per_sleep-self.episode_batch_size):
                ...
                for plot_idx in range(3):
                    ...
                    plt.grid()
                    plt.ylim(0, 1)
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_probs.png'), bbox_inches='tight')
                plt.close()

                # Image probs mean
                plt.figure(figsize=(4, 4))
                image_probs_mean = F.softmax(batch_logits_aggregated[ep_idx], dim=0).detach().cpu().numpy()
                plt.bar(np.arange(len(image_probs_mean)), image_probs_mean)
                plt.title(f'Mean Probabilities')
                plt.xlabel('Cluster ID')
                plt.ylabel('Probability')
                plt.xticks(ticks=np.arange(len(image_probs_mean)), labels=np.arange(len(image_probs_mean)))
                plt.grid()
                plt.ylim(0, 1)
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_probs_aggregated.png'), bbox_inches='tight')
                plt.close()
                ...
                for plot_idx in range(3):
                    ...
                    plt.grid()
                    plt.ylim(0, 1)
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_probs_sharp.png'), bbox_inches='tight')
                plt.close()

                # Image probs mean sharp
                plt.figure(figsize=(4, 4))
                image_probs_mean_sharp = F.softmax(batch_logits_aggregated[ep_idx]/self.tau_s, dim=0).detach().cpu().numpy()
                plt.bar(np.arange(len(image_probs_mean_sharp)), image_probs_mean_sharp)
                plt.title(f'Mean Sharp Probabilities')
                plt.xlabel('Cluster ID')
                plt.ylabel('Probability')
                plt.xticks(ticks=np.arange(len(image_probs_mean_sharp)), labels=np.arange(len(image_probs_mean_sharp)))
                plt.grid()
                plt.ylim(0, 1)
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_probs_aggregated_sharp.png'), bbox_inches='tight')
                plt.close()

                # Image pseudolabel
                plt.figure(figsize=(4, 4))
                image_pseudolabel = batch_labels[ep_idx].detach().cpu().numpy()
                ...
                plt.grid()
                plt.ylim(0, 1)
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_pseudolabel.png'), bbox_inches='tight')
                plt.close()
                ...
                for plot_idx in range(3):
                    ...
                    plt.axis('off')
                plt.savefig(os.path.join(save_dir_images, f'taskid_{task_id}_episode_{current_episode_idx}_image_views.png'), bbox_inches='tight')
                plt.close()

        #### Traack train metrics ####
