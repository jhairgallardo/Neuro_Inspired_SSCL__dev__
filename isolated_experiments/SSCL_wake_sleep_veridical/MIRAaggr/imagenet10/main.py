...
from loss_functions import ConsistLossCARLViewExpanded, KoLeoLoss, ConsistLossMSEViewExpanded, ConsistLossSKLViewExpanded
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_consistencyskl = ConsistLossSKLViewExpanded(num_views = args.num_views).to(device)
        ...
        ### Print time
        print(f'\nTask {task_id} Time: {time.strftime("%H:%M:%S", time.gmtime(time.time() - start_time))} -- Elapsed Time: {time.strftime("%H:%M:%S", time.gmtime(time.time()-init_time))}')
