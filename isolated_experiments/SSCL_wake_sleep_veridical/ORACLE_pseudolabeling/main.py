...
import utils
from models import *
from loss_functions import TwistLossViewExpanded, KoLeoLossViewExpanded, ConsistLossCARLViewExpanded
...
parser.add_argument('--num_pseudoclasses', type=int, default=30)
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        criterion_twist = TwistLossViewExpanded(num_views = args.num_views).to(device)
        criterion_koleoproj = KoLeoLossViewExpanded(num_views = args.num_views).to(device)
        criterion_crossentropy = torch.nn.CrossEntropyLoss().to(device)
        criterion_consistcarl = ConsistLossCARLViewExpanded(num_views = args.num_views).to(device)
        ...
        WS_trainer.sleep_phase(num_episodes_per_sleep = args.num_episodes_per_sleep,
                               optimizer = optimizer, 
                               criterions = [criterion_twist, criterion_koleoproj, criterion_crossentropy, criterion_consistcarl, criterion_bce], 
                               scheduler = scheduler,
                               device = device,
