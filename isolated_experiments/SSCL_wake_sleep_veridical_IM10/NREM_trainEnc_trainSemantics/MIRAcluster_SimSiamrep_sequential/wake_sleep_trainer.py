        ...
        while self.sleep_episode_counter < self.num_episodes_per_sleep:
            ...
            for v in range(self.num_views):
                ...
                with autocast():
                    ...
                    # batch_logits = self.semantic_memory(batch_predictions)
                    batch_logits = self.semantic_memory(batch_targets)
...
def evaluate_semantic_memory(loader,
        ...
        with torch.no_grad():
            for i, (images, targets, _ ) in enumerate(loader):
                ...
                projector_representation = projector_rep(enc_representation)
                # predictor_representation = predictor_rep(projector_representation)
                # logits = semantic_memory(predictor_representation)
                logits = semantic_memory(projector_representation)
