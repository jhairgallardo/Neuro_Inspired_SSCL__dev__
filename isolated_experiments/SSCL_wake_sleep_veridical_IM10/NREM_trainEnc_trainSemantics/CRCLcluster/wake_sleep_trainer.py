...
class Wake_Sleep_trainer:
    ...
    def sleep(self,
        ...
        optimizer_enc = optimizers[0]
        scheduler_enc = schedulers[0]
        optimizer_sem = optimizers[1]
        scheduler_sem = schedulers[1]
        criterion_sem = criterions[0]
        ...
        while self.sleep_episode_counter < self.num_episodes_per_sleep:
            ...
            for v in range(self.num_views):
                ...
                with autocast():
                    ...
                    batch_probs = self.semantic_memory(batch_tensors)
            ...
            for v in range(self.num_views-1):
                loss_sem += criterion_sem(batch_episodes_probs[:,0], batch_episodes_probs[:,v+1])
            loss_sem /= self.num_views
            ### -> Total Loss ###
            loss = loss_sem
...
def evaluate_semantic_memory(loader,
        ...
        with torch.no_grad():
            for i, (images, targets, _ ) in enumerate(loader):
                ...
                probs = semantic_memory(view_encoder(images))
