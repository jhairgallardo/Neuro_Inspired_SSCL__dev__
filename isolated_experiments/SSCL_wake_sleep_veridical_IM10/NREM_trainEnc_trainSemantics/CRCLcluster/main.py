...
from models import *
from loss_functions import CRCL_PCLoss
...
parser = argparse.ArgumentParser(description='Semantic Memory training analysis - CRCLcluster')
# Dataset parameters
...
parser.add_argument('--sem_lr', type=float, default=0.001)
...
def main():
    ...
    for task_id in range(len(train_tasks)):
        ...
        ######-- SLEEP PHASE --######
        print("\n######-- Sleep Phase --######")
        optimizer_enc = torch.optim.AdamW(view_encoder.parameters(), lr = args.enc_lr, weight_decay = args.enc_wd)
        scheduler_enc = torch.optim.lr_scheduler.OneCycleLR(optimizer_enc, max_lr = args.enc_lr, steps_per_epoch = args.num_batch_episodes_per_sleep, epochs=1)
        ...
        criterion_sem = CRCL_PCLoss(batch_size=args.episode_batch_size, alpha=args.alpha, device=device).to(device)
        ### NREM Step ####
