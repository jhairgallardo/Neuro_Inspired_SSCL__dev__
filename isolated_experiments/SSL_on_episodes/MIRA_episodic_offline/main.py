...
def main():
    ...
    criterion = SwapLossViewExpanded(num_views=args.num_views).to(device)
...
def train_step(args, model, train_loader, optimizer, criterion, scheduler, epoch, device, writer=None):
    ...
    for i, batch in enumerate(train_loader):
        ...
        episode_logits = torch.empty(0).to(device)
        for v in range(args.num_views):
            img_batch = episode_imgs[:,v,:,:,:]
            logits = model(img_batch)
            episode_logits = torch.cat([episode_logits, logits.unsqueeze(1)], dim=1)
        
        # MIRA pseudolabeling
        episode_pseudolabels = mira_pseudolabeling(batch_logits = episode_logits, 
        ...
        # loss calculation
        loss = criterion(episode_logits/args.tau_s, episode_pseudolabels)
        ...
        if i % args.print_frequency == 0:
            ps = F.softmax(episode_logits[:,0] / args.tau_s, dim=1).detach().cpu()
            pt = F.softmax(episode_logits[:,0] / args.tau_t, dim=1).detach().cpu()
            _, _, mi_ps = utils.statistics(ps)
            _, _, mi_pt = utils.statistics(pt)
            print(f'Epoch [{epoch}] [{i}/{len(train_loader)}] -- lr: {scheduler.get_last_lr()[0]:.6f}' +
                  f' -- mi_ps: {mi_ps.item():.6f} -- mi_pt: {mi_pt.item():.6f}' +
                  f' -- CrossEntropySwap Loss: {loss.item():.6f}')
            if writer is not None:
                writer.add_scalar('CrossEntropySwap Loss', loss.item(), epoch*len(train_loader)+i)
        # scheduler step      
        scheduler.step()
