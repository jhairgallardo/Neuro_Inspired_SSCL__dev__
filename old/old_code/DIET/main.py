...
parser.add_argument('--init_head_batch', action='store_true')
...
def main():
    ...
    args.save_dir = os.path.join(args.save_dir, 
                                 args.run_name, 
                                 time.strftime("%y%m%d_%H%M%S") + f'{"_noaug" if args.noaug else ""}')
...
def main_worker(args):
    ...
    if args.init_head:
        ...
        model.eval() # model.train() # if I use train mode here, the weights are closer features during training (if no aug is used)
        for x, labels, indexes in train_loader:
            x = x.cuda(args.gpu, non_blocking=True)
            indexes = indexes.cuda(args.gpu, non_blocking=True)
            with torch.no_grad():
                if args.dp: 
                    feats = model.module.encoder(x)
                    model.module.linear_head.weight.data[indexes] = feats.data
                else: 
                    feats = model.encoder(x)
                    model.linear_head.weight.data[indexes] = feats.data

    # # get imgs
    # x_, labels_, indexes_ = next(iter(train_loader))
    # # eval mode
    # model.eval()
    # with torch.no_grad():
    #     a = model(x_.cuda(args.gpu, non_blocking=True))
    #     print(a[:,indexes_])
    #     feat = model.encoder(x_.cuda(args.gpu, non_blocking=True))
    #     print(feat[0])
    # # train mode
    # model.train()
    # a = model(x_.cuda(args.gpu, non_blocking=True))
    ...
    # feat = model.encoder(x_.cuda(args.gpu, non_blocking=True))
    # print(feat[0])
    # # Previously saved features in weights are not the same becuase of augmentations.
    # # Features also won't match saved ones when doing model.train() because of BN in iid case (random batches changes the mean and variance in BN)
    # # if I initalized weights using train mode, then the features are closer to the saved ones during training (if no aug is used)
...
def train(args, epoch, loader, model, optimizer, criterion, scheduler):
    ...
    for x, labels, indexes in loader:
        ...
        if args.init_head_batch:
            output = model(x, indexes)
        else: 
            output = model(x)
...
class DIET(nn.Module):
    ...
    def forward(self, x, indexes=None):
        ...
        if self.args.init_head_batch:
            self.linear_head.weight.data[indexes] = x.data
