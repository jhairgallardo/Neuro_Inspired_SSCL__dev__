...
def main():
    ...
    for class_idx in range(args.num_classes):
        ...
        for task_idx, class_indices_per_task in enumerate(class_indices_per_task):
            train_tasks_array_ID[class_indices_per_task] = task_idx
    ...
    if args.is_main:
        ...
        eval_classification_performance(view_encoder, classifier, val_loader_ID, criterion_sup, writer, 
                                        "Val_ID", args.ddp, device, WS_trainer.total_num_seen_episodes)
        eval_classification_performance(view_encoder, classifier, val_loader_OOD, criterion_sup, writer, 
                                        "Val_OOD", args.ddp, device, WS_trainer.total_num_seen_episodes)
    ...
    if args.is_main:
        # print Total number of tasks and the number of samples per task
        print(f'\n==> Total number of tasks: {args.num_tasks}')
        num_samples_per_task = len(train_tasks[0])
        print(f'==> Number of episodes loaded per task: {num_samples_per_task}')
        if args.num_tasks != args.num_tasks_to_run:
            print(f'==> I am not running all tasks, I am running only the first {args.num_tasks_to_run} tasks')
        else:
            print(f'==> Number of tasks to run: {args.num_tasks}')
