...
class Episode_Transformations:
    ...
    def _random_crop_image(self, base_img: Image.Image, p_crop: float, fix_center_crop=None) -> Image.Image:
        crop_flag = False
        if torch.rand(1) < p_crop: # Apply crop
            ...
            if fix_center_crop is None: # Sample random crop
            ...
            else: # Define a fixed center crop using the fix_center_crop value as the zoom
                zoom = fix_center_crop
    ...
    def __call__(self, pil_img: Image.Image):
        ...
        for i in range(self.num_views):
            ...
            ## Crop
            if i ==0: # First view (It is a big crop on the center of the image [if crop is applied]. No other augs)
                random_big_crop_zoom = random.uniform(0.7, 0.8)
                view, action_cropbb, crop_flag = self._random_crop_image(base_img, self.p_crop, fix_center_crop = random_big_crop_zoom)
                if crop_flag:
