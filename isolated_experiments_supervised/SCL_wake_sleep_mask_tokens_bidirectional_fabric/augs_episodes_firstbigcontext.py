...
class Episode_Transformations:
    ...
    def __call__(self, original_image):
        ...
        for i in range(self.num_views):
            ...
            if i == 0: # first view contains a large context of the original image
                scale = (0.7, 0.8) # large crop of the original image
            else:
                scale = self.scale
            ...
            view, action_cropbb, crop_flag = self.random_resized_crop(original_image_224, # Augmentations are done to the original image (anchor)
                                                           p_crop = self.p_crop, # 90% chance of cropping. 10% chance to not crop and pass the original image as it is to next augmentations
                                                           size = self.size,
                                                           scale = scale,
                                                           ratio = self.ratio)
            ...
            if i == 0: # first view doesn't contain other augmentations (It is only a large crop of the original image)
...
if __name__ == "__main__":
    ...
    grid_img.save("plots/episode_grid_firstaug.png")
