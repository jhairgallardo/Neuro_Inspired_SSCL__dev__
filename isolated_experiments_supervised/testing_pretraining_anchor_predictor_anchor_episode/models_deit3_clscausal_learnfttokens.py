...
class ConditioningNetwork(nn.Module):
    def __init__(self, 
        ...
        # pos-encodings
        self.num_learnable_ft_tokens = 196
        self.pe_img = SinCosPE(feature_dim, n_img_tokens + self.num_learnable_ft_tokens)
        ...
        # learnable ft tokens
        self.learnable_ft_tokens = nn.Parameter(torch.zeros(1, self.num_learnable_ft_tokens, feature_dim))
        trunc_normal_(self.learnable_ft_tokens, std=0.02)
    ...
    def forward(self, feature_tokens_input, aug_seq_batch):
        ...
        tgt = self.feature_mlp(feature_tokens_input) # (B,196,d)

        # 3) Expand learnable ft tokens to batch
        B = tgt.size(0)
        learnable_ft_tokens = self.learnable_ft_tokens.expand(B, -1, -1) # (B,196,d)

        # 4) Concatenate learnable ft tokens with tgt
        tgt = torch.cat((tgt, learnable_ft_tokens), dim=1) # (B,196+196,d) = (B,392,d)

        # 5) add PE for image tokens
        tgt = tgt + self.pe_img(tgt.size(1))                              # (B,392,d)

        # 6) cross-attention
        out = self.transformer_decoder(tgt=tgt,
                                    memory=memory) # (B,392,d)
        
        # 7) get the last self.num_learnable_ft_tokens tokens as output (the learnable ft tokens)
        out = out[:, -self.num_learnable_ft_tokens:, :] # (B,196,d)
        return out
