...
from utils import get_imgpath_label_task_from_domainInc, time_duration_print, file_broadcast_list, plot_generated_images_hold_set
...
parser.add_argument('--num_tasks_to_run', type=int, default=5) # it includes the pretraining task
...
def main():
    ...
    ### Define num_tasks_to_run if not set
    ...
    ### Move models to device, and use DDP if needed
    ...
    for task_id in range(1, args.num_tasks_to_run):
        ...
        while WS_trainer.sleep_episode_counter < args.num_episodes_per_sleep:
            ...
            if args.is_main:
                ...
                results_metrics_per_task = eval_classification_performance(view_encoder, classifier, val_loader, criterion_sup, writer, 
                                    "Val", args.ddp, device, WS_trainer.total_num_seen_episodes, task_id)
            
            ### REM
            if args.REMskip:
                if args.is_main:
                    print(f"=== REM - sleep cycle {nrem_rem_cycle_counter} skipped")
                ...
                if args.is_main:
                    print(f'Validation metrics (REM skipped)')
                    total_num_seen_episodes_reduced = WS_trainer.total_num_seen_episodes*torch.distributed.get_world_size() if args.ddp else WS_trainer.total_num_seen_episodes
                    results_metrics_per_task = eval_classification_performance(view_encoder, classifier, val_loader, criterion_sup, writer, 
                                        "Val", args.ddp, device, WS_trainer.total_num_seen_episodes, task_id)
                    writer.add_scalar('NREM_REM_indicator', WS_trainer.rem_indicator, total_num_seen_episodes_reduced)
            else:
                if args.is_main:
        ...
        # Save results metrics per task
        if args.is_main:
            with open(os.path.join(args.save_dir, f'results_learned_task_{task_id}.json'), 'w') as f:
                json.dump(results_metrics_per_task, f, indent=2)
