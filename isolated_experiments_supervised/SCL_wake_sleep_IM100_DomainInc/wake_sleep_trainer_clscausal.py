...
def eval_classification_performance(view_encoder, 
    ...
    for task_id in all_unique_task_ids:
        ...
        R = f'R_{current_task_id}{task_id}'
    ...
    all_tasks_results['total_num_seen_episodes'] = total_num_seen_episodes_reduced
    

    ### --- Save results for each task --- ###
    for task_id in all_unique_task_ids:
        R = f'R_{current_task_id}{task_id}'
        print(f"{dataset_name}_{R} -- " +
              f"Loss_Sup: {all_tasks_results['loss_sup'][R]:.6f}, " +
              f"Acc@1: {all_tasks_results['acc1'][R]:.2f}, " +
              f"Acc@5: {all_tasks_results['acc5'][R]:.2f}")
        writer.add_scalar(f'{dataset_name}_Task{task_id}_Loss_Sup', all_tasks_results['loss_sup'][R], total_num_seen_episodes_reduced)
        writer.add_scalar(f'{dataset_name}_Task{task_id}_Acc@1', all_tasks_results['acc1'][R], total_num_seen_episodes_reduced)
        writer.add_scalar(f'{dataset_name}_Task{task_id}_Acc@5', all_tasks_results['acc5'][R], total_num_seen_episodes_reduced)


    #### --- Print and save performance for Seen Tasks (including current task) --- ####
    loss_sup_seen_tasks = [all_tasks_results['loss_sup'][f'R_{current_task_id}{taskid}'] for taskid in np.arange(current_task_id+1)]
    acc1_seen_tasks = [all_tasks_results['acc1'][f'R_{current_task_id}{taskid}'] for taskid in np.arange(current_task_id+1)]
    acc5_seen_tasks = [all_tasks_results['acc5'][f'R_{current_task_id}{taskid}'] for taskid in np.arange(current_task_id+1)]
