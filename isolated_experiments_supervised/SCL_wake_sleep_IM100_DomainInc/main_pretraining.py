...
from utils import MetricLogger, reduce_tensor, accuracy, time_duration_print, get_imgpath_label_task_from_domainInc
...
# Dataset parameters
parser.add_argument('--data_path', type=str, default='/home/jhair/datasets/Create_ImageNet100_domain_inc/ImageNet-100_domain_inc_dinov3_vits16plus')
...
parser.add_argument('--save_dir', type=str, default="output/Pretrained/run_debug")
...
def validation_step(val_loader, view_encoder, classifier, val_criterion_sup, device, learned_task):

    all_tasks_results = {'acc1': {}, 'acc5': {}, 'loss_sup': {}}
    ...
    # Forward pass
    with torch.no_grad():
        for i, (batch_imgs, batch_labels, batch_task_ids) in enumerate(val_loader):
    ...
    # Calculate Metrics per task (reduce metrics if using DDP)
    ...
    for task_id in all_unique_task_ids:
        ...
        task_loss_sup = val_criterion_sup(task_logits, task_labels).mean()
        R = f'R_{learned_task}{task_id}'
        # add the results to the all_tasks_results dictionary
        all_tasks_results['acc1'][R] = task_acc1.item()
        all_tasks_results['acc5'][R] = task_acc5.item()
        all_tasks_results['loss_sup'][R] = task_loss_sup.item()
...
def main():
    ...
    train_imgpaths, train_labels_array, train_tasks_array = get_imgpath_label_task_from_domainInc(traindir, args.class_idx_file)
    val_imgpaths, val_labels_array, val_tasks_array = get_imgpath_label_task_from_domainInc(valdir, args.class_idx_file)
    # Get datasets and loaders
    train_dataset_continuum = InMemoryDataset(train_imgpaths, train_labels_array, train_tasks_array, data_type=TaskType.IMAGE_PATH)
    val_dataset_continuum = InMemoryDataset(val_imgpaths, val_labels_array, val_tasks_array, data_type=TaskType.IMAGE_PATH)
    train_tasks = ContinualScenario(train_dataset_continuum, transformations=[train_transform])
    val_tasks = ContinualScenario(val_dataset_continuum, transformations=[val_transform])
    train_dataset = train_tasks[0] # Create the train dataset taking only the first task (the first domain)
    val_dataset = val_tasks[0] # Create the val dataset taking only the first task (the first domain)
    if args.ddp:
    ...
    ### Get validation all tasks loader
    if args.local_rank == 0:
        val_dataset_alltasks = val_tasks[:]
        val_loader_alltasks = torch.utils.data.DataLoader(val_dataset_alltasks, batch_size=args.episode_batch_size_per_gpu, shuffle=False,
    ...
    #######################################
    if args.local_rank == 0:
        all_tasks_results = validation_step(val_loader_alltasks, 
                                            view_encoder.module if args.ddp else view_encoder, 
                                            classifier.module if args.ddp else classifier, 
                                            val_criterion_sup, device, learned_task=-1)
        print(f'Results matrix before training: {all_tasks_results["acc1"]}')
        # Save results
        with open(os.path.join(args.save_dir, 'results_learned_task_-1.json'), 'w') as f:
            json.dump(all_tasks_results, f, indent=2)
    if args.ddp:
    ...
    if args.local_rank == 0:
        all_tasks_results = validation_step(val_loader_alltasks, 
                                            view_encoder.module if args.ddp else view_encoder, 
                                            classifier.module if args.ddp else classifier, 
                                            val_criterion_sup, device, learned_task=0)
        print(f'Results matrix after training on Task 0: {all_tasks_results["acc1"]}')
        # Save results
        with open(os.path.join(args.save_dir, 'results_learned_task_0.json'), 'w') as f:
            json.dump(all_tasks_results, f, indent=2)
