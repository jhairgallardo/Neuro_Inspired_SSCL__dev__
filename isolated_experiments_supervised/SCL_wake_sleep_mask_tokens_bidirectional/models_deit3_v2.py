...
class View_Predictor_Network(nn.Module):
    def __init__(self, 
        ...
        self.clstok_mlp_in    = nn.Linear(clstok_dim, self.hidden_dim)
        ...
        self.clstok_mlp_out   = nn.Sequential(nn.Linear(self.hidden_dim, clstok_dim),
                                              nn.LayerNorm(clstok_dim, eps=1e-6))

        ### Type embeddings per token type
        self.type_emb_imgfttok = nn.Parameter(torch.zeros(self.hidden_dim))
        self.type_emb_clstok = nn.Parameter(torch.zeros(self.hidden_dim))
        self.type_emb_acttok = nn.Parameter(torch.zeros(self.hidden_dim))
        trunc_normal_(self.type_emb_imgfttok, std=0.02)
        trunc_normal_(self.type_emb_clstok, std=0.02)
    ...
    def forward(self, noflat_imgfttoks, noflat_clstok, noflat_acttok):
        """
        Inputs:
          noflat_imgfttoks: (B, V, Timg, Dimg)
          noflat_clstok:    (B, V, 1,    Dcls)
          noflat_acttok:    (B, V, 1,    Dact)
        Returns:
          noflat_PRED_imgfttoks: (B, V, Timg, Dimg)  predictions for each view's image tokens
          noflat_PRED_clstok:    (B, V, 1,    Dcls)  predictions for each view's cls token
        """

        B, V, Timg, Dimg = noflat_imgfttoks.shape
        _, _, _, Dclstok = noflat_clstok.shape
        ...
        noflat_PRED_clstok = torch.zeros_like(noflat_clstok, device=noflat_clstok.device)
        ...
        noflat_clstok_hidden = self.norm_in(self.clstok_mlp_in(noflat_clstok)) # (B, V, 1, Dhidden)
        ...
        noflat_clstok_hidden = noflat_clstok_hidden + self.type_emb_clstok.reshape(1,1,1,Dhidden).expand(B,V,1,Dhidden) # (B, V, 1, Dhidden)
        ...
        base_seqs = torch.cat((noflat_acttok_hidden, noflat_imgfttoks_hidden, noflat_clstok_hidden), dim=2) # (B, V, 1+Timg+1, Dhidden)
        base_seqs = base_seqs.reshape(B, V*(1+Timg+1), Dhidden) # (B, V*(1+Timg+1), Dhidden)

        # 4) Pre-compute positional encoding
        pe = self.pe(base_seqs.size(1)) # (1, V*(1+Timg+1), Dhidden)

        # 5) Normalize mask token and expand
        mask_tokens = self.norm_in(self.mask_token).reshape(1,1,Dhidden)

        # 6) Transformer encoder forward pass per view
        view_len = 1 + Timg + 1                 # [ACT] + [IMG_TOKS x Timg] + [CLS]
        L = V * view_len                        # total sequence length
        for v in range(V):
            start = v * view_len
            img_slice = slice(start + 1, start + 1 + Timg)   # indices for IMG tokens of view v
            cls_slice = slice(start + 1 + Timg, start + 1 + Timg + 1)  # single CLS position

            # Clone base_seqs
            seqs = base_seqs.clone()
            # Replace current view's IMG+CLS slots with mask tokens (plus their PE)
            seqs[:, img_slice, :] = mask_tokens.expand(B,Timg,Dhidden)
            seqs[:, cls_slice, :] = mask_tokens.expand(B,1,Dhidden)
            # Add positional encoding
            seqs = seqs + pe

            # Encode
            seqs_out = self.transformer_encoder(seqs)  # (B, L, Dhidden)

            # Collect predictions at the masked positions
            pred_img_hidden = seqs_out[:, img_slice, :]      # (B, Timg, Dhidden)
            pred_cls_hidden = seqs_out[:, cls_slice, :]      # (B, 1,    Dhidden)

            # Project back to original dims and store
            noflat_PRED_imgfttoks[:, v, :, :] = self.imgfttok_mlp_out(pred_img_hidden) # (B, Timg, Dimg)
            noflat_PRED_clstok[:, v, :, :]    = self.clstok_mlp_out(pred_cls_hidden) # (B, 1, Dclstok)

        return noflat_PRED_imgfttoks, noflat_PRED_clstok
...
class Generator_Network(nn.Module):
    def __init__(self,
        ...
        # It works when using ImageNet statistics (mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])
        ...
        # It works (testing still) when using -1 to 1 scale (mean = [0.5, 0.5, 0.5], std = [0.5, 0.5, 0.5])
