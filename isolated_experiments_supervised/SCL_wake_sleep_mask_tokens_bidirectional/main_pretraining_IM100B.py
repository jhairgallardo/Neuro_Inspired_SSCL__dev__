...
def main():
    ...
    view_predictor = eval(args.view_predictor_model_name)(d_model=args.vpred_dim, 
                                                          ...
                                                          clstok_dim=view_encoder.embed_dim,
    ...
    for epoch in range(args.epochs):
        ...
        for i, (batch_episodes, batch_labels) in enumerate(train_loader):
            ...
            with (autocast(device_type='cuda', dtype=torch.float16)):
                ...
                noflat_PRED_imgfttoks, noflat_PRED_clstok = view_predictor(noflat_imgfttoks, noflat_clstok, noflat_acttok) # (B, V, Timg, Dimg), (B, V, 1, D)
                ...
                # noflat_PRED2_clstok = noflat_PRED2_clstok_and_imgfttoks[:, :, 0:1, :] # (B, V, 1, D)
                ...
                # CE loss
                flat_PRED_clstok = noflat_PRED_clstok.reshape(B * V, noflat_PRED_clstok.size(2), -1).squeeze(1) # (B*V, D)
                flat_logits = classifier(flat_PRED_clstok)
        ...
        with torch.no_grad():
            for j, (batch_episodes, batch_labels) in enumerate(val_loader):
                ...
                with autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    noflat_PRED_imgfttoks, noflat_PRED_clstok = view_predictor(noflat_imgfttoks, noflat_clstok, noflat_acttok)
        ...
        if args.local_rank == 0:
            if (epoch+1) % 1 == 0 or epoch==0:
                ...
                with torch.no_grad():
                    ...
                    noflat_clstok = noflat_feats[:, :, 0:1, :] # (N, V, 1, D)
                    ...
                    # View Predictor forward pass
                    noflat_PRED_imgfttoks, noflat_PRED_clstok = view_predictor(noflat_imgfttoks, noflat_clstok, noflat_acttok) # (N, V, Timg, Dimg), (N, V, 1, D)
