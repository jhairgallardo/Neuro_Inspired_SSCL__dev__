...
class Wake_Sleep_trainer:
    ...
    def REM_sleep(self,
        ...
        while self.sleep_episode_counter < self.num_episodes_per_sleep:
            ...
            with (autocast()):
                ...
                flat_out_tensors = view_encoder(flat_gen_imgs)  # (B*(V-1), T, D)
                flat_cls = flat_out_tensors[:, 0, :]  # Get cls token from all views (B*(V-1), D)
                flat_logits = classifier(flat_cls)       # (B*(V-1), num_classes)               # (B*(V-1),)
                ## Classifier losses and accuracy
    ...
    def condgenertor_LOGAN_forward_pass(self, cond_generator, view_encoder, classifier, 
        ...
        # step memory
        alpha = 10 # 10 0.9   # step size ##################################################################### ALPHA (step size for action vector optimization)
        with torch.no_grad():
            # memory = memory - alpha * grad_mem # Descent step on memory (make samples easier to classify)
            memory = memory + alpha * grad_mem # Ascent step on memory (make samples harder to classify)
