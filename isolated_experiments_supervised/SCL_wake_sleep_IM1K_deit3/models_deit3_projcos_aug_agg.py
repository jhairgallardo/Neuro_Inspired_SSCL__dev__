...
class Classifier_Model(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.projector(x) # Projector
        x = self.norm(x) # L2 normalization
        w = self.norm(self.classifier_head.weight)
        o = (x @ w.t()) / self.tau
...
class AugEncoder(nn.Module):
    def __init__(self, d_model=64, n_layers=2, n_heads=4, dim_ff=256):
        ...
        # ## Register: aug register token (similar to a cls token)
        # self.aug_register_token = nn.Parameter(torch.zeros(1, 1, d_model))  # (1,1,D)
        # trunc_normal_(self.aug_register_token, std=.02)

    def forward(self, x, pad_mask):             # (B,L,D),(B,L)
        B, L, D = x.size()

        ### Register
        # # Expand the aggregator token to match batch size
        # aug_register_token = self.aug_register_token.expand(x.size(0), -1, -1)
        # # Concatenate the aggregator token to the input sequence
        # x = torch.cat([aug_register_token, x], dim=1) # (B,L+1,D)
        # # Update the padding mask to account for the aggregator token
        # pad_agg = torch.zeros(B, 1, dtype=torch.bool, device=pad_mask.device)
        # pad_mask = torch.cat([pad_agg, pad_mask], dim=1)  # (B,L+1)
        # # Pass through the encoder
        # x = self.enc(x, src_key_padding_mask=pad_mask) # (B,L+1,D)
        # # Extract the aggregator token output. The rest of the sequence is not needed, so we can ignore it
        # output = x[:, 0:1, :]  # (B,1,D)

        ### Mean (only take the mean of the non padded tokens or "valid" tokens) (Mean A)
        # x = self.enc(x, src_key_padding_mask=pad_mask) # (B,L,D)
        # # Build a float mask for valid (non-padded) tokens
        # valid_mask = (~pad_mask).unsqueeze(-1).type_as(x)  # (B, L, 1)
        # # Zero out padded positions
        # x = x * valid_mask                                 # (B, L, D)
        # # Sum and divide by counts of valid tokens
        # sum_embeddings = x.sum(dim=1, keepdim=True)        # (B, 1, D)
        # valid_counts   = valid_mask.sum(dim=1, keepdim=True).clamp(min=1e-9)  # (B, 1, 1)
        # # Mean-pooled output
        # output = sum_embeddings / valid_counts

        # ### Normal way (Use it also for MeanB)
        # output = self.enc(x, src_key_padding_mask=pad_mask) # (B,L,D)

        # ### Causal Transformer (Take last token)
        causal_mask = torch.nn.Transformer.generate_square_subsequent_mask(L).to(x.device) # (T,T)
        output = self.enc(x, src_key_padding_mask=pad_mask, mask=causal_mask) # (B, T, D)
        # Take the last valid token (the last token in the sequence that is not a padding token)
        last_valid_idx = (~pad_mask).sum(dim=1) - 1  # (B,)
        output = output[torch.arange(B), last_valid_idx, :]  # (B, D)
...
class ConditioningNetwork(nn.Module):
    def __init__(self, 
        ...
        self.dim_type_emb = 32 # 32 #4 #aug_feature_dim // 2
        self.dim_linparam = 32 # 32 #16 #aug_feature_dim // 2
        ...
        # V0
        # assert self.dim_type_emb + self.dim_linparam == aug_feature_dim, "aug_feature_dim must be equal to dim_type_emb + dim_linparam"
        # self.pe_aug = SinCosPE(self.dim_type_emb, max_aug_tokens) 


        # # V1 aug side
        # self.aug_mlp_in = nn.Linear(self.dim_type_emb + self.dim_linparam, aug_feature_dim)
        # self.pe_aug = SinCosPE(aug_feature_dim, max_aug_tokens)  


        # V2
        self.augtype_mlp = nn.Linear(self.dim_type_emb, aug_feature_dim // 2) # (D_type,D_augtype)
        self.augparam_mlp = nn.Linear(self.dim_linparam, aug_feature_dim // 2) # (D_linparam,D_augparam)
        self.pe_aug = SinCosPE(aug_feature_dim // 2, max_aug_tokens) # only add pos to the first half of the token (type embedding section)


        # aug encoder
        self.aug_enc = AugEncoder(d_model=aug_feature_dim, n_layers=aug_n_layers, 
                                    n_heads=aug_n_heads, dim_ff=aug_dim_ff)
        self.aug_mlp_out = nn.Sequential(
    ...
    def forward(self, feature_tokens_input, aug_seq_batch):
        ...
        ### V1
        # aug_tok = self.aug_mlp_in(aug_tok)
        # aug_tok = aug_tok + self.pe_aug(aug_tok.size(1))     # (B,L,D)

        ### V2
        aug_tok_type = aug_tok[:, :, :self.dim_type_emb]  # (B,L,D_type)
        aug_tok_params = aug_tok[:, :, self.dim_type_emb:]  # (B,L,D_linparam)
        aug_tok_type = self.augtype_mlp(aug_tok_type)
        aug_tok_type = aug_tok_type + self.pe_aug(aug_tok_type.size(1))
        aug_tok_params = self.augparam_mlp(aug_tok_params)
        aug_tok = torch.cat([aug_tok_type, aug_tok_params], dim=2)  # (B,L,D)

        # Encode the augmentation tokens
        memory  = self.aug_enc(aug_tok, pad_mask)            # (B,1,D) or (B,L,D) if using all tokens
        memory  = self.aug_mlp_out(memory)                       # (B,1,d) or (B,L,d)

        # ## Mean B (after mlp out)
        # # Build a float mask for valid (non-padded) tokens
        # valid_mask = (~pad_mask).unsqueeze(-1).type_as(memory)  # (B, L, 1)
        # # Zero out padded positions
        # memory = memory * valid_mask                                 # (B, L, D)
        # # Sum and divide by counts of valid tokens
        # sum_embeddings = memory.sum(dim=1, keepdim=True)        # (B, 1, D)
        # valid_counts   = valid_mask.sum(dim=1, keepdim=True).clamp(min=1e-9)  # (B, 1, 1)
        # # Mean-pooled output
        # memory = sum_embeddings / valid_counts
        ...
        # 3) cross-attention
        if memory.shape[1] > 1:
            out = self.transformer_decoder(tgt=tgt, memory=memory, memory_key_padding_mask=pad_mask) # (B,196,d) If using all tokens, otherwise memory_key_padding_mask is None
        else:
            out = self.transformer_decoder(tgt=tgt, memory=memory) # (B,196,d)
