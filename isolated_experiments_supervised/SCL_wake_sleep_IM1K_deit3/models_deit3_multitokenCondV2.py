...
class AugTokenizerSparse(nn.Module):
    def __init__(self, d_model=24):
        ...
        self.type_emb = nn.Embedding(len(self.name2id), d_model)

        self.proj = nn.ModuleDict({
            "crop"  : nn.Linear(4, d_model), # process 4 params
            "hflip" : None,                  # no params to process
            "jitter": nn.Linear(7, d_model), # process 7 params
            "gray"  : None,                  # no params to process
            "blur"  : nn.Linear(1, d_model), # process 1 param
            "solar" : nn.Linear(1, d_model), # process 1 param
    ...
    def _tok(self, name, params):
        ...
        if head is not None and params.numel():
            t = t + head(params.to(dev).unsqueeze(0))        # (1,D)
        return t
    ...
    def forward(self, batch_aug_lists):
        ...
        returns padded_tokens (B,Lmax,D) , pad_mask (B,Lmax)
        """
...
class AugEncoder(nn.Module):
    def __init__(self, d_model=24, n_layers=2, n_heads=8, dim_ff=512):
        super().__init__()
