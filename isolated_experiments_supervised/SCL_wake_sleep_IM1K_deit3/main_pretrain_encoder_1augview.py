...
def main():
    ...
    train_transform = Transformations(mean = args.mean, std = args.std)
    ...
    #     for i in range(5):
    #         imgs = batch_imgs[:6]
    #         labels = batch_labels[:6]
    #         grid = vutils.make_grid(imgs, nrow=6, padding=2, normalize=True)
    #         plt.figure(figsize=(12, 8))
    #         plt.imshow(np.transpose(grid.cpu(), (1, 2, 0)))
    #         plt.axis('off')
    #         plt.show()
    #         plt.savefig(os.path.join(args.save_dir, f'example_images.png'), bbox_inches='tight', dpi=300)
    #         plt.close()
    ...
    for epoch in range(args.epochs):
        ...
        for i, (batch_imgs, batch_labels, _) in enumerate(train_loader):
            batch_imgs = batch_imgs.to(device, non_blocking=True) # (B, C, H, W)
            batch_labels = batch_labels.to(device, non_blocking=True) # (B)
