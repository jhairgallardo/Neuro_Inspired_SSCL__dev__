...
class ConditioningNetwork(nn.Module):
    def __init__(self, 
        ...
        # # Per-Patch Expansion: summary_dim → 196×feature_dim
        # self.expand_linear = nn.Linear(feature_dim, n_img_tokens * feature_dim)
        # self.post_norm = nn.LayerNorm(feature_dim, eps=1e-6)
    ...
    def forward(self, feature_tokens_input, aug_seq_batch):
        ...
        # # 4) per‐patch expansion MLP → (B,196,feature_dim)
        # B, L, D = aug_tok.shape
        # s      = memory.squeeze(1)                          # (B, feature_dim)
        # z_flat = self.expand_linear(s)                      # (B,196*feature_dim)
        # z      = z_flat.view(B, -1, tgt.size(-1))           # (B,196,feature_dim)
        # tgt = tgt + z                                        # inject variation
        # tgt = self.post_norm(tgt)                            # (B,196,d)  2. Normalize after adding

        # 4) add PE for image tokens
