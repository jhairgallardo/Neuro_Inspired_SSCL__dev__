...
class ConditioningNetwork(nn.Module):
    def __init__(self, 
        ...
        # Transformer encoder (no cross-attention)
        enc_layer = nn.TransformerEncoderLayer(
    ...
    def forward(self, feature_tokens_input, aug_seq_batch):
        ...
        # 2) embed + PE for image tokens (= encoder queries)
        tgt = self.feature_mlp(feature_tokens_input)

        # 3) add PE for image tokens
        tgt = tgt + self.pe_img(tgt.size(1))                              # (B,196,d) # 196 is the number of image tokens

        # 4) Instead of cross-attention, just concatenate the memory and the target and pass it through the transformer encoder
        tgt_with_memory = torch.cat([tgt, memory], dim=1) # (B,196+1,d)
        out = self.transformer_encoder(tgt_with_memory) # (B,196+1,d)
        out = out[:, :-1, :] # (B,196,d)
        return out
