...
class Episode_Transformations:
    def __init__(self, 
        ...
        # center crop to size value function for first view
        self.center_crop = transforms.Compose([transforms.Resize((self.size,self.size)),
                                              transforms.CenterCrop((145, 145)), # scale of center crop is 145/224 = 0.6473214285714286 ~ 0.65
    ...
    def __call__(self, original_image):
        ...
        ########################################################
        original_image_224 = self.resize_only(original_image)
        first_view = self.center_crop(original_image_224) # First view is a center crop of the original image
        ...
        for i in range(1, self.num_views):
            ...
            ## Random Resized Crop
            view, action_cropbb, crop_flag = self.random_resized_crop(original_image_224, # Augmentations are done to the original image (anchor)
                                                           p_crop = self.p_crop,
                                                           size = self.size,
                                                           scale = self.scale,
                                                           ratio = self.ratio)
            if crop_flag: # Translation happened
                tokens.append(("crop", action_cropbb))

            # Note: using the first view if crops are not used hinders the ability to look at other views when information is missing. The network
            # starts liking looking at the first view more in the sequence. This is why I am using crops all the time (that do not contain parts of the first view necesarrily)
            # so the network has to look at other views besides the first view to be able to reconstruct the current view features
        ...
        # Final action is an empty list (No action, since there is no next view to predict)
        aug_seq.append([])
