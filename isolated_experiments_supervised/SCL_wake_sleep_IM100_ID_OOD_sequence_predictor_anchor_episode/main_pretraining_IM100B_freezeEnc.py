        ...
        for i, (batch_episodes, batch_labels) in enumerate(train_loader):
            ...
            with (autocast(device_type='cuda', dtype=torch.float16)):
                ...
                with frozen_params(view_encoder):
                    flat_pred_feats_from_pred_imgs = view_encoder(flat_pred_imgs)[:, 1:, :] # (B*(V-1), Timg, Dimg) (discard the cls token)
                ...
                with frozen_params(view_encoder):
                    flat_pred_feats_from_pred_imgs_skipcond = view_encoder(flat_pred_imgs_skipcond)[:, 1:, :] # (B*V, Timg, Dimg) (discard the cls token)
