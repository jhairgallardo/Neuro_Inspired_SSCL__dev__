...
from augs_insequence_hallucinate import Episode_Transformations, collate_function_notaskid
...
@torch.no_grad()
def ema_update(ema, online, m):
    online = _raw(online)
    for pe, po in zip(ema.parameters(), online.parameters()):
        pe.data.mul_(m).add_(po.data, alpha=1.0 - m)

def ema_momentum(step, total_steps, m_min=0.992, m_max=0.9995):
    # ramp m from m_min to m_max over the first ~80% of training
    t = min(step / (0.8 * total_steps), 1.0)
    # cosine from m_min → m_max
    return m_max - (m_max - m_min) * 0.5 * (1.0 + math.cos(math.pi * t))
...
def main():
    ...
    # ---------------- EMA teacher for generator losses ----------------
    view_encoder_ema = copy.deepcopy(view_encoder).to(device).eval()
    for p in view_encoder_ema.parameters():
        p.requires_grad_(False)  # teacher never gets gradients
    ...
    for epoch in range(args.epochs):
        ...
        for i, (batch_episodes, batch_labels) in enumerate(train_loader):
            ...
            with (autocast(device_type='cuda', dtype=torch.float16)):
                ...
                flat_feats_and_cls = view_encoder(flat_imgs) # (B*V, 1+Timg, D)
                all_feats = flat_feats_and_cls.view(B, V, flat_feats_and_cls.size(1), -1) # (B, V, 1+Timg, Dimg)


                #### Conditional Generator (involves view encoder)####
                # Get feature tokens and action codes (both no flattened)
                noflat_fts = all_feats[:, :, 1:, :] # (B, V, Timg, Dimg)
                noflat_actions = batch_episodes_actions # (B,V,A) list length B, each elemnt has V actions. Actions A vary for each view.
                #[ [batch_episodes_actions[b][v] for v in range(V)] for b in range(B)]
                Timg = noflat_fts.size(2)
                Dimg = noflat_fts.size(3)

                ### Run the conditional generator (predicting next view)
                # We ommit the last view because there is no next view to predict for it (input are views 0,1,2. Output are predicted views 1,2,3)
                noflat_pred_imgs, noflat_pred_fts = cond_generator(noflat_fts[:, :-1, :], [noflat_actions[b][:-1] for b in range(B)]) # (B, V-1, C, H, W), (B, V-1, Timg, Dimg)
                flat_pred_fts = noflat_pred_fts.reshape(B * (V-1), Timg, Dimg) # (B*(V-1), Timg, Dimg) Predicted next views (1,2,3)
                flat_target_fts = noflat_fts[:, 1:, :].reshape(B * (V-1), Timg, Dimg) # (B*(V-1), Timg, Dimg) Targets are the next views (1,2,3)
                loss_gen1 = criterion_condgen(flat_pred_fts, flat_target_fts.detach())
                # Forward pass the next views predicted images through the view encoder and calculate loss
                flat_pred_imgs = noflat_pred_imgs.reshape(B * (V-1), C, H, W) # (B*(V-1), C, H, W)
                flat_pred_feats_from_pred_imgs = view_encoder_ema(flat_pred_imgs)[:, 1:, :] # (B*(V-1), Timg, Dimg) (discard the cls token)
                loss_gen2 = criterion_condgen(flat_pred_feats_from_pred_imgs, flat_target_fts.detach())

                ### Run generator without conditioning (skip conditioning_network). It helps training the upsampling resnet faster (from tokens to generated images)
                # Here, we can use all views because we are not predicting next views. We are just generating images from the feature tokens.
                noflat_pred_imgs_skipcond = cond_generator(noflat_fts, None, skip_conditioning=True) # (B, V, C, H, W)
                flat_pred_imgs_skipcond = noflat_pred_imgs_skipcond.reshape(B * V, C, H, W) # (B*V, C, H, W)
                flat_pred_feats_from_pred_imgs_skipcond = view_encoder_ema(flat_pred_imgs_skipcond)[:, 1:, :] # (B*V, Timg, Dimg) (discard the cls token)
                flat_target_fts_skipcond = noflat_fts.reshape(B * V, Timg, Dimg) # (B*V, Timg, Dimg)
                loss_gen3 = criterion_condgen(flat_pred_feats_from_pred_imgs_skipcond, flat_target_fts_skipcond.detach())


                ### Classifier (involves view encoder) ###
                # get cls tokens from view encoder output
                ...
                # C) Using all views (reverse order so first view is only seens no the final token)
                    ...
                    notflat_cls = notflat_cls.flip(dims=[1])  # Reverse the order of views
                    batch_episodes_labels = batch_episodes_labels.flip(dims=[1])  # Reverse the order of labels
                    notflat_sup_logits = classifier(notflat_cls, first_token_droprate=args.cls_firstviewdroprate)
                ...
                Vcls = notflat_sup_logits.size(1) # Number of views for the classifier
                sup_logits = notflat_sup_logits.reshape(B * Vcls, -1) # (B*Vcls, num_classes)
                sup_labels = batch_episodes_labels.reshape(-1) # (B*Vcls,)
                loss_sup  = criterion_sup(sup_logits, sup_labels)

                loss_sup = loss_sup.view(B, Vcls)
                # first view has less weight (alpha is the fraction from the uniform weight) (reduction should be none for this)
                w = make_token_weights_fractional(Vcls, alpha=args.firstviewCEweight, device=sup_logits.device, dtype=sup_logits.dtype)  # sum=1
            ...
            # View_encoder and classifier step, and view encoder EMA update
            ...
            current_ema_m = ema_momentum(global_step, total_steps)
            ema_update(view_encoder_ema, view_encoder, m=current_ema_m) 
            # Next view generator (condgen) step
        ...
        if (args.local_rank == 0) and (((epoch+1) % 10) == 0) or epoch==0:
            ...
            view_encoder_ema_state_dict = view_encoder_ema.state_dict() # EMA encoder is not under ddp
            ...
            torch.save(view_encoder_ema_state_dict, os.path.join(args.save_dir, f'view_encoder_ema_epoch{epoch}.pth'))
        ...
        if args.local_rank == 0:
            if (epoch+1) % 1 == 0 or epoch==0:
                ...
                classifier.eval()
                N = 16
                episodes_plot_imgs = episodes_plot[0][:N].to(device, non_blocking=True) # (B, V, C, H, W)
                episodes_plot_actions = episodes_plot[1][:N] # (B, V, A)
                _, V, C, H, W = episodes_plot_imgs.shape
                with torch.no_grad():
                    flat_feats = view_encoder(episodes_plot_imgs.reshape(N * V, C, H, W))[:, 1:, :] # Discard the CLS token. Shape is (B, Timg, Dimg)
                    Timg, Dimg = flat_feats.shape[1], flat_feats.shape[2]
                    noflat_feats = flat_feats.reshape(N, V, Timg, Dimg) # (N, V, Timg, Dimg) 
                    noflat_actions = episodes_plot_actions # (N, V, A)
                    gen_images, _ = cond_generator(noflat_feats[:, :-1, :, :], [noflat_actions[b][:-1] for b in range(N)]) # (B, V-1, C, H, W) Discard the last view in inputs (no next view to predict for it. It has an empty action)
                episodes_plot_gen_imgs = gen_images.detach().cpu() # (B, V-1, C, H, W)
                episodes_plot_imgs = episodes_plot_imgs.detach().cpu() # (B, V, C, H, W)
                # plot each episode
                for i in range(N):
                    ...
                    episode_i_gen_imgs = torch.stack(episode_i_gen_imgs, dim=0) # (V-1, C, H, W)
                    episode_i_gen_imgs = torch.clamp(episode_i_gen_imgs, 0, 1) # Clip values to [0, 1]

                    # Add a black image as first view for episode_i_gen_imgs
                    zeros = torch.zeros_like(episode_i_gen_imgs[:1]) # (1, C, H, W)
                    episode_i_gen_imgs = torch.cat([zeros, episode_i_gen_imgs], dim=0) # (V, C, H, W)
        ...
        # ### Plot reconstructions examples (Autoregressive)###
        # if args.local_rank == 0:
        #     if (epoch+1) % 1 == 0 or epoch==0:
        #         view_encoder.eval()
        #         cond_generator.eval()
        ...
        #             actions = [episodes_plot_actions[b][:-1] for b in range(n)] # Discard the last view action (no next view to predict for it. It has an empty action)
        #             gen_images, _ = cond_generator.autoregressive_forward(first_view_tensors, actions) # (B, V-1, C, H, W)
        #         episodes_plot_gen_imgs = gen_images.detach().cpu() # (B, V-1, C, H, W)
        #         episodes_plot_imgs = episodes_plot_imgs.detach().cpu() # (B, V, C, H, W)
        #         # plot each episode
        #         for i in range(n):
