        ...
        for i, (batch_episodes_imgs, batch_labels, _) in enumerate(train_loader):
            ...
            with (autocast()):
                for v in range(1, args.num_views):
                    ...
                    loss_koleo_view = criterion_koleo(batch_tensors.mean(dim=(2,3))) # pass the average pooled vector to Koleo (koleo works on vectors)
        ...
        for i, (batch_imgs, batch_labels, _) in enumerate(val_loader):
            with torch.no_grad():
                ...
                with autocast():
                    ...
                    loss_koleo = criterion_koleo(batch_tensors.mean(dim=(2,3))) # pass the average pooled vector to Koleo (koleo works on vectors)
                loss_total = loss_sup + args.koleo_gamma * loss_koleo
