...
class Episode_Transformations:
    ...
    def __call__(self, original_image):
        ...
        if self.return_actions:
            action_vectors = torch.zeros(self.num_views, 7)
        ...
        if self.return_actions:
            action_vectors[0] = torch.tensor([0., 0., 0., 1., 1., 1., 0.]) 
        ...
        for i in range(1, self.num_views):
            ...
            action_grayscale = 0
            action_gaussianblur = 0
            action_solarization = 0
            choice = random.randint(0, 2)
            if choice == 0:
                view, action_grayscale = self.apply_random_grayscale(first_view, p_grayscale = self.p_grayscale)
            elif choice == 1:
                view, action_gaussianblur = self.apply_gaussian_blur(first_view, p_gaussian_blur = self.p_gaussian_blur)
            elif choice == 2:
                view, action_solarization = self.apply_solarization(first_view, p_solarization = self.p_solarization)
            
            view, action_colorjitter = self.color_jitter(view, p_color_jitter = self.p_color_jitter, 
            ...
            if self.return_actions: 
                action_vectors[i] = torch.tensor([action_grayscale, 
                                                  action_gaussianblur, 
                                                  action_solarization,
