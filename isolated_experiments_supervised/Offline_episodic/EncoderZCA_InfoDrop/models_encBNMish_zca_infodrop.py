...
class Info_Dropout(nn.Module):
    ...
    def forward(self, x_old, x, use_infodrop=False):
        if not use_infodrop:
            return x
        with torch.cuda.amp.autocast(enabled=False):
            with torch.no_grad():
                ...
                distance = torch.cat(distances, dim=1).float() # make sure FP32 for InfoDrop calculations. Doing mixed precision here can lead to numerical instability
                ...
                n_samples = int(self.drop_rate * h * w)
                
                # --- Compute the expected keep ratio (deterministic scaling factor) ---
                # For each spatial location, the expected mask value is: (1 - prob)^(n_samples)
                expected_keep_mask = (1 - prob) ** n_samples
                # Average over the spatial dimensions to get per-sample, per-channel expected keep ratio:
                expected_keep_ratio = expected_keep_mask.mean(dim=(-2, -1))  # shape: (B, C)
                expected_keep_ratio = expected_keep_ratio.unsqueeze(-1).unsqueeze(-1)  # shape: (B, C, 1, 1)
                # Optionally, clip to avoid extremely small ratios causing huge scaling:
                expected_keep_ratio = expected_keep_ratio.clamp(min=1e-2)

                # --- Generate the random binary dropout mask ---
                device = x.device
                random_choice = random_sample(prob, sampling_num=n_samples)
                random_mask = torch.ones((batch_size * channels, h * w), device=device)
                random_mask[torch.arange(batch_size * channels, device=device).view(-1, 1), random_choice] = 0
                mask = random_mask.view(x.shape).float()

                # --- Apply mask and scale by the deterministic expected keep ratio ---
                x_scaled = x * mask / (expected_keep_ratio + 1e-8)

                # ### Scale actions according to the keep_ratio (this is so I can remove the infodropout layer during inference)
                # # I am scaling across the batch and channels
                # mask = random_mask.view(x.shape).float()  # mask shape: (B, C, H, W)
                # keep_ratio = mask.view(mask.size(0), mask.size(1), -1).mean(dim=-1)  # shape: (B, C)
                # keep_ratio = keep_ratio.unsqueeze(-1).unsqueeze(-1)  # shape: (B, C, 1, 1)
                # x_scaled = x * mask / (keep_ratio + 1e-8)  # added a small epsilon for numerical stability

                ### no scale
                # x_scaled = x * random_mask.view(x.shape)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.if_infodropout = if_infodropout
        if self.if_infodropout:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.if_infodropout:
            out = self.info_dropout1(x_old, out, use_infodrop=self.training)  # InfoDrop is only used during training (self.training flag)
        ...
        if self.downsample is None and self.if_infodropout:
            out = self.info_dropout2(x_old, out, use_infodrop=self.training)  # InfoDrop is only used during training (self.training flag)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], if_infodropout=(infodrop_blocks>=1))
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0], if_infodropout=(infodrop_blocks>=2))
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1], if_infodropout=(infodrop_blocks>=3))
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], if_infodropout=(infodrop_blocks>=4))
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_classes)
            

        # Initialize weights and normalization layers
    ...
    def _make_layer(
        ...
        layers.append(
            block(
                self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer, if_infodropout
            )
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.infodrop_blocks > 0: # If info_dropout layer exists
            x = self.info_dropout(x_old, x, use_infodrop=self.training)  # InfoDrop is only used during training (self.training flag)
