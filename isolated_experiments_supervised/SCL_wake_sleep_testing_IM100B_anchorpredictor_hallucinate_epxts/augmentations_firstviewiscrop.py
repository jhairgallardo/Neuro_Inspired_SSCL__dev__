...
class Episode_Transformations:
    ...
    def __call__(self, original_image):
        ...
        aug_seq.append([]) # Empty list for no action. (It will use a "None" token later)
        ...
        for i in range(1, self.num_views):
            ...
            ## Random Resized Crop
            view, action_cropbb, crop_flag = self.random_resized_crop(original_image_224, # Augmentations are done to the original image
                                                           p_crop = self.p_crop, # 1.0, all views will be cropped
                                                           size = self.size,
                                                           scale = self.scale, # (0.2, 0.8), none of the views will show the complete image
                                                           ratio = self.ratio)
            # if crop_flag:
